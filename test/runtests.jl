...
c = MetaConfigurations.load_file("test.yml") # load the test file
...
c = MetaConfigurations.load_file("test.yml", a=3, b="5", c=[1, 2, 3]) # load with additional properties
@test c["a"] == 3
@test c["b"] == "5"
@test c["c"] == [1, 2, 3]

# test writing to files by parsing the output back again
c = MetaConfigurations.load_file("test.yml")
filename = tempname() * ".yml"
YAML.write_file(filename, c) # write to temporary file
parsed = MetaConfigurations.load_file(filename)
rm(filename) # cleanup
@test parsed == c
